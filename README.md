...
- **Hero rarity tiers** (Common/Rare/Epic/Legendary) across a 16-hero roster,
  with 4 new heroes and abilities (Dash-cooldown reduction, checkpoint
  spacing, daily-streak bonus, an all-rounder).
- **Story-mode checkpoints**: dying mid-stage now lets you Retry from your
  last checkpoint instead of the very start (checkpoint spacing tunable per
  hero ability).
- **Pet evolution**: pets earn XP from every run's coin haul, leveling up
  through Hatchling → Adult → Ascended, each level boosting their radius and
  attack speed slightly.
- **Ghost replay**: your best-ever Endless run is recorded and played back
  as a faint translucent echo on future Endless runs.
- **Share Score** button on the Game Over screen (native share sheet where
  supported, clipboard fallback otherwise).
- **Screenshot Mode**: a HUD toggle that hides all overlay chrome for a
  clean screenshot.
- **Frame-rate cap** (30/60) in Settings — 30fps mode halves simulation
  work for battery saving on older phones.
- **Three-phase intro**: studio logo card → loading screen with rotating
  gameplay tips → the existing cinematic title.

**Honest notes on this pass:** a literal "global leaderboard" needs a
server/database and isn't included; "70+ handcrafted levels" would mean 70
individually hand-designed levels, which is real level-design work beyond
what a data-driven system can fake — Story Mode instead scales cleanly
(21 stages across 7 worlds) and can grow further by adding stage entries.
"Wall Jump" still doesn't fit this flat-runner genre (no walls to jump off);
Perfect Jump Bonus remains the closest fit and was already implemented.
