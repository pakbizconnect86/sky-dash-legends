...
function makeWeatherParticles(kind, W, H){
  ...
  for (let i=0;i<n;i++){
    arr.push({
      ...
      speed: kind==='rain' ? 700+Math.random()*300 : 40+Math.random()*40,
      drift: kind==='snow' ? (Math.random()-0.5)*30 : 0,
      len: kind==='rain' ? 14+Math.random()*10 : 0,
      r: kind==='snow' ? 1.5+Math.random()*2 : 0
    });
  }
  return arr;
}
...
function renderWorld(g, wr, W, H, GROUND_Y, scrollSpeed, dt){
  ...
  // parallax hills/buildings, tinted by world + darker at night
  const hillColor = lerpColor('#78a0d2', '#1c2947', night);
  g.fillStyle = hillColor + (night>0.5?'':'');
  g.globalAlpha = 0.4;
  wr.hills.forEach(hl=>{
